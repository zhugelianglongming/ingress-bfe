...
func (p *Processor) preProcessIngresses(ingresses ingressList) {
...
func (p *Processor) Start() {
	if p.client == nil || p.ingressCh == nil {
		panic(fmt.Errorf("start process without client of ingress chan"))
	}
	...
	for {
		...
		case ingresses := <-p.ingressCh:
			...
			p.preProcessIngresses(ingresses)
...
func NewProcessor(c *kubernetes_client.KubernetesClient, ingressCh chan ingressList, stopCh chan struct{}) *Processor {
	return &Processor{
		client:       c,
		ingressCh:    ingressCh,
		stopCh:       stopCh,
